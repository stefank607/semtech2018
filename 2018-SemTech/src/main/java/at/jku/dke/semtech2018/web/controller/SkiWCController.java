...
public class SkiWCController {
	...
	public String greeting(Model model) {
		...
		try {
			// Query q = QueryFactory.create("SELECT ?s ?p ?o ?g WHERE {{?s ?p ?o} UNION
			// {GRAPH ?g {?s ?p ?o}}}");
			// Query q = QueryFactory.create("SELECT ?s ?o WHERE { ?s a
			// <http://example.org/Skifahrer>; <http://example.org/hatGewonnen> ?o}");
			Query q = QueryFactory.create(
					"SELECT ?nn ?vn ?wc WHERE { ?s <http://example.org/hatGewonnen> ?o; <http://example.org/hatNachnamen> ?nn; <http://example.org/hatVornamen> ?vn. ?o <http://example.org/hatBezeichnung> ?wc}");
			try (QueryExecution qEx = QueryExecutionFactory.create(q, d)) {
				...
				for (QuerySolution qs : list) {
					listStr.add(qs.getLiteral("?vn").toString() + " " + qs.getLiteral("?nn").toString() + " ("
							+ qs.getLiteral("?wc").toString() + ")");
				}
	...
	public String getDetail(@PathVariable("detail") String detail, Model model) {
		...
		if (result.length >= 2) {
			...
			org.apache.jena.rdf.model.Model rdfmod = RDFDataMgr
					.loadModel("https://kgsearch.googleapis.com/v1/entities:search?query=" + result[0] + "+" + result[1]
							+ "&key=AIzaSyBv-_PDRKuF2aYcnfjjWa9HxgXxIrEg_h0&limit=1&indent=True", Lang.JSONLD);

			Query q = QueryFactory.create(
					"SELECT ?o WHERE {{ ?s <http://schema.org/articleBody> ?o } UNION { ?s <http://schema.org/description> ?o}}");
